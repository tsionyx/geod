...
mod partial_try_from {
    ...
    impl TryFrom<[i16; 4]> for Longitude<DegreeAngle> {
...
mod tests {
    ...
    #[test]
    fn size_long() {
        assert_eq!(size_of::<Longitude<DegreeAngle>>(), 4);
    }

    fn east_meridians() -> Vec<Longitude<DegreeAngle>> {
        ...
        let deg_160_1 = Longitude::with_angle_and_direction(
            DegreeAngle::with_dms(160, 15, 1, 845).unwrap(),
            East,
        )
        .unwrap();
    ...
    fn west_meridians() -> Vec<Longitude<DegreeAngle>> {
    ...
    fn prime_is_always_less() {
        let p = Longitude::<DegreeAngle>::prime();
        east_meridians()
            .iter()
            .skip(1)
    ...
    fn anti_is_always_greater() {
        let anti = Longitude::<DegreeAngle>::anti_meridian();
        east_meridians()
            .iter()
            .rev()
            .skip(1)
    ...
    fn equal_prime() {
        ...
        let prime3 = Longitude::with_angle_only(DegreeAngle::zero()).unwrap();
        let prime4 =
            Longitude::with_angle_and_direction(DegreeAngle::with_dms(0, 0, 0, 0).unwrap(), East)
                .unwrap();
        let prime5 = Longitude::with_angle_and_direction(DegreeAngle::complete(), West).unwrap();
    ...
    fn equal_anti_meridian() {
        ...
        let anti3 = Longitude::with_angle_and_direction(DegreeAngle::straight(), East).unwrap();
        let anti4 = Longitude::with_angle_and_direction(DegreeAngle::straight(), West).unwrap();
        let anti5 = Longitude::with_angle_only(DegreeAngle::straight()).unwrap();
    ...
    fn bad_longitude_east() {
        let _l = Longitude::<DegreeAngle>::east(200).unwrap();
    }

    #[test]
    #[should_panic(expected = "ReflexAngle")]
    fn bad_longitude_east_2() {
        let _l = Longitude::<DegreeAngle>::try_from([200, 0]).unwrap();
    }

    #[test]
    fn good_longitude_west() {
        let l: Longitude<DegreeAngle> = [-180, 0].try_into().unwrap();
    ...
    fn opposite_longitudes() {
        let l = Longitude::<DegreeAngle>::east((50, 14, 21)).unwrap();
        let opp = l.opposite();
    ...
    #[test]
    fn west_0_is_prime() {
        let p = Longitude::<DegreeAngle>::west(0).unwrap();
        let a = Longitude::anti_meridian();
        assert_eq!(p.opposite(), a);
        assert_eq!(a.opposite(), p);
        assert_eq!(p.angle(), DegreeAngle::zero())
    }

    #[test]
    fn from_f64_east() {
        let l: Longitude<DegreeAngle> = 41.622_512.try_into().unwrap();
    ...
    #[test]
    fn from_f64_west() {
        let l = Longitude::<DegreeAngle>::try_from(-84.120_456).unwrap();
    ...
    fn from_f64_overflow() {
        let _l = Longitude::<DegreeAngle>::try_from(190.622_512).unwrap();
    }
}
...
mod parse_tests {
    ...
    #[test]
    fn simple_degree() {
        let l: Longitude<DegreeAngle> = "15° E".parse().unwrap();

        assert_eq!(l.direction(), Some(East));
        assert_eq!(l.angle(), DegreeAngle::try_from(15).unwrap());
    }

    #[test]
    fn suffix_decimal() {
        let l: Longitude<DegreeAngle> = "34.1551784° E".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "34.1551784E".parse().unwrap();
        assert_eq!(l_ascii, l);

        assert_eq!(l.direction(), Some(East));
        assert_eq!(l.angle(), DegreeAngle::try_from(34.155_178_4).unwrap());
    }

    #[test]
    fn suffix_with_space() {
        let l: Longitude<DegreeAngle> = "34°16′22″ E".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "34* 16' 22\" E".parse().unwrap();
        assert_eq!(l_ascii, l);

        assert_eq!(l.direction(), Some(East));
        assert_eq!(l.angle(), DegreeAngle::with_dms(34, 16, 22, 0).unwrap());
    }

    #[test]
    fn suffix_no_space() {
        let l: Longitude<DegreeAngle> = "43° 20′ 7.15″W".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "43 20'7.15\"W".parse().unwrap();
        assert_eq!(l_ascii, l);

        assert_eq!(l.direction(), Some(West));
        assert_eq!(l.angle(), DegreeAngle::with_dms(43, 20, 7, 150).unwrap());
    }

    #[test]
    fn prefix_decimal() {
        let l: Longitude<DegreeAngle> = "W 34.0045°".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "W34.0045".parse().unwrap();
        assert_eq!(l_ascii, l);

        assert_eq!(l.direction(), Some(West));
        assert_eq!(l.angle(), DegreeAngle::try_from(34.0045).unwrap());
    }

    #[test]
    fn prefix_with_space() {
        let l: Longitude<DegreeAngle> = "E 34°16′0″".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "E 34* 16'".parse().unwrap();
        assert_eq!(l_ascii, l);

        assert_eq!(l.direction(), Some(East));
        assert_eq!(l.angle(), DegreeAngle::with_dms(34, 16, 0, 0).unwrap());
    }

    #[test]
    fn prefix_no_space() {
        let l: Longitude<DegreeAngle> = "W89° 0′ 2.44″".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "W89 0'2.44\"".parse().unwrap();
        assert_eq!(l_ascii, l);

        assert_eq!(l.direction(), Some(West));
        assert_eq!(l.angle(), DegreeAngle::with_dms(89, 0, 2, 440).unwrap());
    }

    #[test]
    fn prefix_sign_decimal() {
        let l: Longitude<DegreeAngle> = "-34.0045°".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "-34.0045".parse().unwrap();
        assert_eq!(l_ascii, l);
    ...
    fn prefix_sign_with_space() {
        let l: Longitude<DegreeAngle> = "+34°16′0″".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "+34 16'".parse().unwrap();
        assert_eq!(l_ascii, l);
    ...
    fn prefix_sign_no_space() {
        let l: Longitude<DegreeAngle> = "-89° 0′ 2.44″".parse().unwrap();
        let l_ascii: Longitude<DegreeAngle> = "-89*0'2.44\"".parse().unwrap();
        assert_eq!(l_ascii, l);
    ...
    fn prime_does_not_require_pole() {
        let p1: Longitude<DegreeAngle> = "0°".parse().unwrap();
        let p2: Longitude<DegreeAngle> = "0".parse().unwrap();
        let p3: Longitude<DegreeAngle> = "0 0'0\"".parse().unwrap();
        assert_eq!(p1, p2);
        assert_eq!(p2, p3);
    ...
    fn anti_does_not_require_pole() {
        let p1: Longitude<DegreeAngle> = "180°".parse().unwrap();
        let p2: Longitude<DegreeAngle> = "180.0".parse().unwrap();
        let p3: Longitude<DegreeAngle> = "180 0'0\"".parse().unwrap();
        assert_eq!(p1, p2);
        assert_eq!(p2, p3);
...
mod bad_parse_tests {
    ...
    fn no_prefix_no_suffix() {
        let _l: Longitude<DegreeAngle> = "15°".parse().unwrap();
    }

    #[test]
    #[should_panic(expected = "EmptyString")]
    fn empty() {
        let _l: Longitude<DegreeAngle> = "".parse().unwrap();
    }

    #[test]
    #[should_panic(expected = "Angle(DmsNotation)")]
    fn bad_sign() {
        let _l: Longitude<DegreeAngle> = "--89° 16′".parse().unwrap();
    }
    ...
    #[should_panic(expected = "Angle(DmsNotation)")]
    fn bad_float() {
        let _l: Longitude<DegreeAngle> = "-15.46.11".parse().unwrap();
    }

    #[test]
    #[should_panic(expected = "Angle(AngleNotInRange(ReflexAngle))")]
    fn too_big_angle_east() {
        let _l: Longitude<DegreeAngle> = "+190°16′".parse().unwrap();
    }

    #[test]
    #[should_panic(expected = "Angle(AngleNotInRange(ReflexAngle))")]
    fn too_big_angle_west() {
        let _l: Longitude<DegreeAngle> = "-181".parse().unwrap();
    }

    #[test]
    fn round_more_than_7_digits() {
        let l: Longitude<DegreeAngle> = "-18.99999995°".parse().unwrap();
        // 90 - 18.9999999 = 70.0000001
        assert_eq!(
            l.angle().complement().unwrap(),
            DegreeAngle::try_from(71.000_000_1).unwrap()
        );
    }
    ...
    #[should_panic(expected = "NoHemisphere")]
    fn bad_direction() {
        let _l: Longitude<DegreeAngle> = "Z18.15°".parse().unwrap();
    }

    #[test]
    fn more_than_right_angle() {
        let l_east: Longitude<DegreeAngle> = "+123* 34' 42\"".parse().unwrap();
        let l_west: Longitude<DegreeAngle> = "-123 34' 42\"".parse().unwrap();
